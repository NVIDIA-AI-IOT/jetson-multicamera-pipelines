...
class ImageCV:
    ...
    def updateCVImage(self, gstSample):
        ...
        self._imageCV = cv2.imdecode(
            np.frombuffer(imageStream.read(), np.uint8), -1)
...
class ImageStitcher():
    ...
    def homoStitch(self, imageArray=None):
        ...
        kpsA, featuresA = self.detectAndDescribe(
            self._imageArray[0], method='orb')
        kpsB, featuresB = self.detectAndDescribe(
            self._imageArray[1], method='orb')
        matches = self.matchKeyPointsBF(featuresA, featuresB, method='orb')
        M = self.getHomography(kpsA, kpsB, featuresA,
                               featuresB, matches, reprojThresh=4)
        if M is None:
        ...
        self._result = cv2.warpPerspective(
            self._imageArray[0], H, (width, height))
        self._result[0:self._imageArray[1].shape[0],
                     0:self._imageArray[1].shape[1]] = self._imageArray[1]
    ...
    def fast_stitch(self, imageArray: List):
        ...
        self._imageArray = imageArray
        print("hello")
        cv2.imwrite(join(getcwd(), "..", "imageDir",
                    'left.jpg'), self._imageArray[0])
        cv2.imwrite(join(getcwd(), "..", "imageDir",
                    'right.jpg'), self._imageArray[1])
        stitcher = Stitcher()
        self._result = stitcher.stitch(
            ["../imageDir/left.jpg", "../imageDir/right.jpg"])

    def nextImage(self, imageArray=None, saveImage=True, showImage=False, dir=getcwd()) -> None:
        '''
        Gets next image
        '''
    ...
    def saveImage(self, all=True, dir=getcwd()):
        def worker(location, img):
            '''
            Saves image in grey scale with resizing on stitched image
            '''
            try:
        ...
        # indexing of the image
        self._imageCount = (self._imageCount + 1) % 5

        # Concurrent image save to reduce time spent on IO
        if (all):
            jobs = []
            jobs.append(Process(target=worker, args=(join(
                dir, "..", "imageDir", 'stitch{}.jpg'.format(self._imageCount)), self._result)))
            jobs.append(Process(target=worker, args=(join(
                dir, "..", "imageDir", 'left{}.jpg'.format(self._imageCount)), self._imageArray[0])))
            jobs.append(Process(target=worker, args=(join(
                dir, "..", "imageDir", 'right{}.jpg'.format(self._imageCount)), self._imageArray[1])))
            for process in jobs:
                process.start()
            for process in jobs:
                process.join()
        else:
            cv2.imwrite(join(dir, "..", "imageDir", 'stitch{}.jpg'.format(
                self._imageCount)), self._result)
