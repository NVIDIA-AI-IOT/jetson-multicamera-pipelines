...
int main()
{
    ...
    // OpenCV containers
    cv::Mat img_l_3ch, img_r_3ch, img_l, img_r, img_r_rect, img_l_rect;
    ...
    CHECK_STATUS(vpiCreateStereoDisparityEstimator(
        backend, DISP_W, DISP_H, VPI_IMAGE_FORMAT_U16, NULL, &stereo));

    std::string pipeline_right = gstreamer_pipeline(0, CAP_W, CAP_H, DISP_W, DISP_H, FPS, FLIP_METHOD);
    std::string pipeline_left = gstreamer_pipeline(1, CAP_W, CAP_H, DISP_W, DISP_H, FPS, FLIP_METHOD);
    ...
    cv::moveWindow("right", DISP_W, 0); // Align windows side by side
    ...
    CHECK_STATUS(vpiImageCreate(DISP_W, DISP_H, VPI_IMAGE_FORMAT_U16, 0, &disparity));



    std::vector<float> distortion_coef_right = {-0.3725356 ,  0.19930816,  0.00158068, -0.00206056, -0.06456895};
    std::vector<float> distortion_coef_left = distortion_coef_right;

    std::vector<float> intriniscs_right = {
        702.40993198,   0.        , 236.87260342,
        0.        , 696.21049833, 126.35710758,
        0.        ,   0.        ,   1.        };

    std::vector<float> intrinsics_left = intriniscs_right;
    ...
    while (true) {
        ...
        cv::undistort(img_l, img_l_rect, K_l, dist_coeff_l);
        cv::undistort(img_r, img_r_rect, K_r, dist_coeff_r);
        ...
        // uint8 -> uint16
        img_l.convertTo(img_l_rect, CV_16UC1);
        img_r.convertTo(img_r_rect, CV_16UC1);
        ...
        cv::imshow("left", img_l_3ch);
        cv::imshow("right", img_r_3ch);
